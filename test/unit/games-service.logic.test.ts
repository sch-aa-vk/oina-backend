...
const checkQuota = (
  ...
  const effectiveMonthly =
    currentMonthStart !== currentMonth ? 0 : gamesThisMonth;
  if (effectiveMonthly >= MAX_MONTHLY) {
    throw new AppError(
      409,
      "QUOTA_MONTHLY_GAMES_EXCEEDED",
      "Max 3 games per month",
    );
  }
};
...
describe("games service – unit logic", () => {
  describe("quota checks", () => {
    ...
    it("rejects when totalGames reaches 5", () => {
      expect(() => checkQuota(5, 0, currentMonth, currentMonth)).toThrow(
        AppError,
      );
    });
    ...
    it("rejects when gamesThisMonth reaches 3", () => {
      expect(() => checkQuota(2, 3, currentMonth, currentMonth)).toThrow(
        AppError,
      );
    });
  ...
  describe("ownership", () => {
    it("allows owner access", () => {
      ...
      expect(() => {
        if (game.userId !== "user-1")
          throw new AppError(403, "GAME_FORBIDDEN", "Forbidden");
      }).not.toThrow();
    });
  ...
  describe("visibility transitions", () => {
    it("allows draft -> private-link", () => {
      expect(() =>
        assertValidTransition("draft", "private-link"),
      ).not.toThrow();
    });
    ...
    it("allows private-link -> public", () => {
      expect(() =>
        assertValidTransition("private-link", "public"),
      ).not.toThrow();
    });

    it("allows public -> private-link", () => {
      expect(() =>
        assertValidTransition("public", "private-link"),
      ).not.toThrow();
    });

    it("allows private-link -> draft (unpublish)", () => {
      expect(() =>
        assertValidTransition("private-link", "draft"),
      ).not.toThrow();
    });
    ...
    it("allows preview for draft games", () => {
      ...
      expect(() => {
        if (game.visibility !== "draft")
          throw new AppError(400, "PREVIEW_ONLY_FOR_DRAFT", "Not a draft");
      }).not.toThrow();
    });
