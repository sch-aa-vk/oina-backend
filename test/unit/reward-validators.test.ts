...
describe("reward validation", () => {
    ...
    it("rejects text reward missing text field", () => {
      expect(() =>
        validateCreateGamePayload({
          ...
          content: {
            rewards: [
              { id: "r1", pointsThreshold: 100, label: "Great!", type: "text" },
            ],
          },
        }),
      ).toThrow(AppError);
    ...
    it("accepts image reward without assetKey", () => {
      expect(() =>
        validateCreateGamePayload({
          ...
          content: {
            rewards: [
              { id: "r1", pointsThreshold: 200, label: "Star!", type: "image" },
            ],
          },
        }),
      ).not.toThrow();
  ...
  describe("reward count limit", () => {
    const makeReward = (i: number) => ({
      id: `r${i}`,
      pointsThreshold: i * 10,
      label: `Reward ${i}`,
      type: "text" as const,
      text: "msg",
    });
