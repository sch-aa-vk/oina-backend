...
} from "../../src/utils/game-validators";
...
describe("game-validators", () => {
  describe("validateCreateGamePayload", () => {
    ...
    it("rejects missing title", () => {
      expect(() =>
        validateCreateGamePayload({ ...validPayload, title: "" }),
      ).toThrow(AppError);
    });

    it("rejects title over 120 chars", () => {
      expect(() =>
        validateCreateGamePayload({ ...validPayload, title: "a".repeat(121) }),
      ).toThrow(AppError);
    });
  ...
  describe("validateUpdateGamePayload", () => {
    ...
    it("rejects empty title string", () => {
      expect(() => validateUpdateGamePayload({ title: "   " })).toThrow(
        AppError,
      );
    });
    ...
    it("rejects title over 120 chars", () => {
      expect(() =>
        validateUpdateGamePayload({ title: "a".repeat(121) }),
      ).toThrow(AppError);
    });
  });
  ...
  describe("validatePublishGamePayload", () => {
    it("accepts private-link", () => {
      expect(() =>
        validatePublishGamePayload({ visibility: "private-link" }),
      ).not.toThrow();
    });

    it("accepts public", () => {
      expect(() =>
        validatePublishGamePayload({ visibility: "public" }),
      ).not.toThrow();
    });

    it("rejects draft as visibility", () => {
      expect(() =>
        validatePublishGamePayload({ visibility: "draft" as never }),
      ).toThrow(AppError);
    });
