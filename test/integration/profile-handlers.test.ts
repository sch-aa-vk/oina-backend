...
jest.mock("@aws-sdk/s3-request-presigner", () => ({
  getSignedUrl: jest
    .fn()
    .mockResolvedValue(
      "https://presigned.s3.amazonaws.com/avatars/user-1?X-Amz-Signature=abc",
    ),
}));
...
describe("PATCH /users/me", () => {
  it("returns 200 and updated profile after valid partial update", async () => {
    ...
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ displayName: "New Name" }) }),
    );
  ...
  it("updates bio independently", async () => {
    ...
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ bio: "Updated bio" }) }),
    );
  ...
  it("updates username when not taken", async () => {
    ...
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ username: "alice_new" }) }),
    );
  ...
  it("returns 409 when username is already taken by another user", async () => {
    mockSend.mockResolvedValueOnce({
      Items: [{ userId: "other-user", username: "taken_name" }],
    });

    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ username: "taken_name" }) }),
    );
  ...
  it("supports partial payload (only provided fields are updated)", async () => {
    ...
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ bio: "new bio" }) }),
    );

    expect(result.statusCode).toBe(200);
    const body = JSON.parse(result.body);
    expect(body.data.displayName).toBe(mockUser.displayName);
    expect(body.data.bio).toBe("new bio");
  });

  it("returns 400 VALIDATION_ERROR for displayName that is too long", async () => {
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ displayName: "x".repeat(101) }) }),
    );
  ...
  it("returns 400 VALIDATION_ERROR for invalid username format", async () => {
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ username: "ab" }) }),
    );
  ...
  it("returns 401 when token is missing", async () => {
    ...
    const result = await updateMe(
      makeEvent({ headers: {}, body: JSON.stringify({ bio: "test" }) }),
    );
  ...
  it("updates updatedAt timestamp", async () => {
    ...
    const result = await updateMe(
      makeEvent({ body: JSON.stringify({ bio: "something" }) }),
    );
